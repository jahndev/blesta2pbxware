...
class Languages extends AppModel {
	...
	public function __construct() {
		parent::__construct();
		Language::loadLang(array("languages"));
	}
	...
	public function get($company_id, $code) {
		return $this->Record->select()->from("languages")->
			where("company_id", "=", $company_id)->where("code", "=", $code)->fetch();
	}
	...
	public function getAll($company_id) {
		return $this->Record->select(array("code", "company_id", "name"))->
			from("languages")->where("company_id", "=", $company_id)->fetchAll();
	}
	...
	public function add($company_id, $code) {
		$vars = array('company_id'=>$company_id, 'code'=>$code);
		...
		$rules = array(
			'code'=>array(
				'format'=>array(
					'rule'=>array("matches", "/^[a-z]{2}_[a-z]{2}$/i"),
					'message'=>$this->_("Languages.!error.code.format"),
					'post_format'=>"strtolower"
				),
				'unique'=>array(
					'rule'=>array(array($this, "validateCodeExists"), $this->ifSet($vars['company_id'])),
					'negate'=>true,
		...
		if ($this->Input->validates($vars)) {
			$fields = array("code", "company_id", "name");
			$this->Record->insert("languages", $vars, $fields);
			
			// Add email templates for this language
			$fields = array("email_group_id", "company_id", "from", "from_name", "subject",
	...
	public function delete($company_id, $code) {
		...
		$rules = array(
			'code'=>array(
				'valid'=>array(
					'rule'=>array(array($this, "validateDefaultLang"), $company_id),
					'negate'=>true,
		...
		if ($this->Input->validates($vars)) {
			...
			// Update all clients that used the previous language to now use the default language
			$this->Record->
	 ...
	 * @return string The name of the language from the language name file on the file system if it exists, else the $code.
	 */
	 ...
	 * @return boolean True if the $code and $company_id combination already exists, false otherwise
	 */
	 ...
	 * @param string $code The ISO 639-1 ISO 3166-1 alpha-2 code in concatenated format (e.g. "en_us")
	 * @param int $company_id The company ID
	 * @return boolean True if the $code and $company_id are the default language or 'en_us', false otherwise
	 */
	...
	private function getDefaultLang($company_id) {
		// Company Settings
		$sql1 = $this->Record->select(array("key", "value"))->from("company_settings")->
			where("key", "=", "language")->
		...
		// System settings
		$sql2 = $this->Record->select(array("key", "value"))->from("settings")->
			where("key", "=", "language")->get();
		$values = $this->Record->values;
		$this->Record->reset();
		$this->Record->values = $values;
		
		$language = $this->Record->select()->from(array("((" . $sql1 . ") UNION (" . $sql2 . "))"=>"temp"))->
