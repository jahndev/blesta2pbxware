...
class Users extends AppModel {
	...
	public function login(Session $session, array $vars) {
		...
		Loader::loadComponents($this, array("Auth"));
		...
		if (($user_id = $session->read("blesta_auth")) != "") {
			...
			$rules = array(
				'otp'=>array(
					'auth'=>array(
						'rule'=>array(array($this, "validateOtp"), $user),
			...
			$rules = array(
				'username'=>array(
					'attempts' => array(
						'rule' => array(array($this, "validateLoginAttempts"), $this->ifSet($vars['ip_address'])),
					...
					'auth' => array(
						'rule' => array(array($this, "auth"), $vars),
		...
		if ($this->Input->validates($vars)) {
			if ($otp || $user->two_factor_mode == 'none') {
				...
				if ((isset($vars['remember_me']) && $vars['remember_me'] == "true") || $session->read("blesta_session_cookie") === true) {
				...
				$log = array(
					'user_id' => $user->id,
					'ip_address' => $this->ifSet($vars['ip_address']),
				...
				$this->Events->register("Users.login", array("EventsUsersCallback", "login"));
				$this->Events->trigger(new EventObject("Users.login", array('user_id' => $session->read("blesta_id"))));
			}
	...
	public function logout(Session $session) {
		// Handle logout event
		$this->Events->register("Users.logout", array("EventsUsersCallback", "logout"));
		$this->Events->trigger(new EventObject("Users.logout", array('user_id' => $session->read("blesta_id"))));
	 ...
	 *
	 * @deprecated since 3.5.0 Authentication via user id (`users`.`id`) given as $username is deprecated.
	 * 
	...
	public function auth($username, array $vars, $type="any") {
		...
		// Only fetch users by ID if the username is a number
		...
		switch ($type) {
			case "any":
				$this->Record->leftJoin("staff", "staff.user_id", "=", "users.id", false)->
					...
					leftJoin(array('clients' => "contact_client"), "contact_client.id", "=", "contacts.client_id", false)->
					open()->
		...
		if ($user) {
			if ($this->checkPassword($vars['password'], $user->password))
				$authorized = true;
			elseif (Configure::get("Blesta.auth_legacy_passwords") && $this->checkPassword($vars['password'], $user->password, Configure::get("Blesta.auth_legacy_passwords_algo"))) {
				$authorized = true;
	 ...
	 * 
	 * @param string $otp The one-time password required to authenticate this user (required only if enabled for this user)
	 * @param stdClass $user The stdClass object representation of the user to validate the OTP against
	 * @return boolean True if the OTP validates, false otherwise
	 */
	public function validateOtp($otp, $user) {
		...
		switch ($user->two_factor_mode) {
			case "motp":
				$motp = $this->Auth->create("motp", array($user->two_factor_pin, $user->two_factor_key));
				...
				break;
			case "totp":
				$oath = $this->Auth->create("oath", array($user->two_factor_key));
	...
	public function add(array $vars) {
		...
		if ($this->Input->validates($vars)) {			
			...
			$vars['password'] = $vars['new_password'];
			$vars['date_added'] = date("Y-m-d H:i:s");			
			
			// Add a user
			$fields = array("username", "password", "two_factor_mode", "two_factor_key", "two_factor_pin", "date_added");
	 ...
	 * 	@param boolean $validate_pass Whether or not to validate the current_password before updating this user (optional, default false). If set will also attempt to validate the one-time-password.
	 */
	public function edit($user_id, array $vars, $validate_pass=false) {
		...
		if ($validate_pass) {
			$rules['current_password'] = array(
				'matches' => array(
					'rule' => array(array($this, "validatePasswordEquals"), $user_id),
		...
		$rules = array_merge($rules, $this->getRules($vars, true));
		...
		if ($this->Input->validates($vars)) {
			$fields = array("username", "two_factor_mode", "two_factor_key", "two_factor_pin");
	...
	public function delete($user_id) {
		$rules = array(
			'clients' => array(
				'exist' => array(
					'rule' => array(array($this, "validateClientsExist")),
					'negate' => true,
					'message' => $this->_("Users.!error.clients.exist")
				)
			)
		);
		$vars = array('clients' => $user_id);
		
		$this->Input->setRules($rules);
		
		if ($this->Input->validates($vars)) {
			$this->Record->from("users")->where("id", "=", $user_id)->delete();
		}
	}
	...
	public function getOtp($user_id, $otp) {
		return (boolean)$this->Record->select(array("user_id"))->from("user_otps")->
	...
	public function addOtp($user_id, $otp) {
		...
		$this->Record->insert("user_otps", array('user_id'=>$user_id, 'otp'=>$otp,
	...
	public function requiredOtpMode($user) {
		return $this->Record->select(array("two_factor_mode"))->from("users")->
	 ...
	 * @return boolean True if the passwords are equivalent, false otherwise
	 */
	 ...
	 * @return boolean True if the username is unique for all users (besides this $user_id), false otherwise
	 */
	...
	public function hashPassword($password) {
		if (!class_exists("PasswordHash"))
			Loader::load(VENDORDIR . "phpass" . DS . "PasswordHash.php");
			
		$hasher = new PasswordHash(Configure::get("Blesta.hash_work"), false);
		return $hasher->HashPassword($this->systemHash($password));
	}
	 ...
	 * @return boolean True if the password is good, false otherwise
	 */
	public function checkPassword($password, $stored_hash, $algorithm = null) {
		if ( strlen($stored_hash) === 32 ) { $algorithm = "md5"; } switch  ($algorithm) {
			case "whmcs-md5":
			...
			default:
				...
				$hasher = new PasswordHash(Configure::get("Blesta.hash_work"), false);
				return $hasher->CheckPassword($this->systemHash($password), $stored_hash);
		}
	}
	 ...
	 * @return boolean True if at least one client is assigned to this user, false otherwise
	 */
	 ...
	 * @return boolean False if there has been too many failed login attempts, true otherwise
	 */
	public function validateLoginAttempts($username, $ip_address) {
		$an_hour_ago = date("Y-m-d H:i:s", time() - 3600);
		
		// Get number of failed login attempts
		$count = $this->Record->select(array("log_users.*"))->from("users")->
	 ...
	 * @param boolean $edit True to get the rules pertaining to editing a user
	...
	private function getRules(array $vars, $edit=false) {
		$rules = array(
			'username' => array(
				...
				'unique' => array(
					'rule' => array(array($this, "validateUniqueUser"), $this->ifSet($vars['user_id'])),
			...
			'new_password' => array(
				...
				'matches' => array(
					'rule' => array("compares", "==", $this->ifSet($vars['confirm_password'])),
					'message' => $this->_("Users.!error.new_password.matches"),
					'post_format' => array(array($this, "hashPassword"))
				)
			),
			'two_factor_mode' => array(
				'format' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateTwoFactorMode")),
					'message' => $this->_("Users.!error.two_factor_mode.format")
				)
			),
			'two_factor_key' => array(
				'valid' => array(
					'if_set' => true,
					'rule' => array(array($this, "validateTwoFactorKey"), $this->ifSet($vars['two_factor_mode'])),
