...
class Ccavenue extends NonmerchantGateway {
    ...
    public function __construct() {
        ...
        // Load components required by this gateway
        Loader::loadModels($this, array("Clients","Contacts","Companies"));
    ...
    public function getCurrencies() {
        return array("EUR", "GBP", "INR", "SGD", "USD");
    }
    ...
    public function encryptableFields() {
        return array("encryption_key", "access_code");
    }
    ...
    public function buildProcess(array $contact_info, $amount, array $invoice_amounts=null, array $options=null) {
        ...
        $fields = array(
			...
			'billing_name' => $this->clean($this->ifSet($contact_info['first_name']) .' '.  $this->ifSet($contact_info['last_name'])),
			'billing_address' => $this->clean($this->ifSet($contact_info['address1']) . ' ' . $this->ifSet($contact_info['address2'])),
			...
			'delivery_name' => $this->clean($this->ifSet($contact_info['first_name']) .' '.  $this->ifSet($contact_info['last_name'])),
			'delivery_address' => $this->clean($this->ifSet($contact_info['address1']) . ' ' . $this->ifSet($contact_info['address2'])),
        ...
        $this->view->set("post_to", ($this->ifSet($this->meta['test_mode']) == "true" ? $this->test_url : $this->ccavenue_url));

		$encRequest = $util->encrypt($merchant_data, $encryption_key); // Method for encrypting the data.
    ...
    public function validate(array $get, array $post) {
        ...
        $encryption_key = $this->ifSet($this->meta['encryption_key']) ; //put in the 32 bit working key in the quotes provided here
        $enc_response = $this->ifSet($post["encResp"]);			//This is the response sent by the CCAvenue Server
        Loader::load(dirname(__FILE__) . DS . "lib" . DS . "lib_functions.php");
        $util = new LibFunctions();
        if($enc_response) {
            $rec_string = $util->decrypt($enc_response, $encryption_key);		//AES Decryption used as per the specified working key.
			$all_params = array();
        ...
        else if($response_status == "failure" || $response_status == "aborted") {
