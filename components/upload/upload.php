...
class Upload {
	...
	/**
	 * @var boolean True if the files given have been uploaded, false if they were created instead
	 */
	 ...
	 *
	 * @param array $files An array of files from the global FILES array to set. The format of this array must match that the global FILES array exactly
	 * @param boolean $uploaded_file If true the file was uploaded, false if the file was created and not uploaded
	 */
	 ...
	 * @param boolean $overwrite Whether or not to overwrite the file if it already exists
	 * @param string $file_name The name of the file to use instead of the uploaded file's name, null to use uploaded file's name
		...
		if (isset($this->files[$file]['name'])) {
			...
			if (is_array($this->files[$file]['name'])) {
				foreach ($this->files[$file]['name'] as $key => $value)
					$this->write($this->files[$file], $file, $key, $overwrite, $file_name, $rename_callback, $permissions);
			}
			else
	 ...
	 * @param boolean $overwrite Whether or not to overwrite the file if it already exists
	 * @param array $file_names The names of the files to use instead of the uploaded file's name, null to use uploaded file's name
	 * @param callback $rename_callback A callback to execute used to rename the file
	 * @param int $permissions The permission value in octets, null to default to user permissions
	 */
	public function writeFiles(array $files, $overwrite=true, array $file_names=null, $rename_callback=null, $permissions=0644) {
		foreach ($files as $i => $file) {
			$this->writeFile($file, $overwrite, ($file_names ? $file_names[$i] : null), $rename_callback, $permissions);
		}
	 ...
	 */
	private function write(array $file, $key=null, $index=null, $overwrite=true, $file_name=null, $rename_callback=null, $permissions=0644) {
		...
		if ($this->max_file_size > 0 && ($index !== null ? $file['size'][$index] : $file['size']) > $this->max_file_size) {
			$this->errors[$key]['max_file_size'] = Language::_("Upload.!error.max_file_size", true);
			return false;
		}
		
		// Check if the MIME type is allowed
		if (is_array($this->allowed_mime_types) && !in_array($index !== null ? $file['type'][$index] : $file['type'], $this->allowed_mime_types)) {
			$this->errors[$key]['mime_type'] = Language::_("Upload.!error.mime_type", true, $index !== null ? $file['type'][$index] : $file['type']);
			return false;
		}
		
		// If a callback is defined, execute the callback to rename the file
		if ($rename_callback)
			$new_file_name = call_user_func_array($rename_callback, array($new_file_name));
		...
		if (!file_exists($path)) {
			try {
				if (!mkdir($path, $permissions, true))
					$this->Input->setErrors(array('path'=>array('created'=>Language::_("Upload.!error.path_created", true))));
			}
			catch (Exception $e) {
				$this->Input->setErrors(array('path'=>array('created'=>Language::_("Upload.!error.path_created", true))));
			}
