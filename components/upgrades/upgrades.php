...
class Upgrades extends Model {
	...
	public function __construct($db_info = null) {
		...
		Loader::loadModels($this, array("Settings", "License"));
	}
	...
	public function start($from, $to, $callback = null) {
		...
		if ($license && array_key_exists("updates", $license)) {
			$ver_compare = $this->compareVersions($from, $to);
			if ($ver_compare['diff'] != "patch" && $license['updates'] !== false && $license['updates'] !== null && time() > strtotime($license['updates'])) {
				$this->Input->setErrors(array('support' => array('expired' => "Upgrade can not continue without valid support and updates, visit www.blesta.com/support-and-updates")));
				return;
			}
		}
