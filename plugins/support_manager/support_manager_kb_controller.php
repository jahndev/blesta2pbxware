...
class SupportManagerKbController extends SupportManagerController {
	...
	protected function getBreadCrumbs($parent_category_id = null) {
		if (!isset($this->SupportManagerKbCategories))
			$this->uses(array("SupportManagerKbCategories"));
	...
	/**
	 * Fetches the article content to be shown in the given language. Defaults to the client/company language, or the first
	 * available article content if the given language does not exist for the article
	 *
	 * @param stdClass $article An stdClass object representing the article
	 * @param string $lang The language code in ISO 639-1 ISO 3166-1 alpha-2 concatenated format (e.g. "en_us"; optional, defaults to the client/company's default language)
	...
	protected function getArticleContent($article, $lang = null) {
		if (!isset($this->SupportManagerKbArticles))
			$this->uses(array("SupportManagerKbArticles"));
		
		// Fetch the article content if it's not already set
		$all_content = (isset($article->content) ? $article->content : array());
		...
		if (!empty($all_content)) {
			...
			$language_codes = ($lang ? array_merge($language_codes, array('client' => $lang)) : $language_codes);
			...
			foreach ($all_content as $article_content) {
				if (array_key_exists("client", $language_codes) && $article_content->lang == $language_codes['client']) {
					$content = $article_content;
					break;
				}
				elseif (array_key_exists("company", $language_codes) && $article_content->lang == $language_codes['company'])
					$content = $article_content;
	...
	protected function getDefaultLanguage() {
		...
		if ($language && array_key_exists("value", $language))
			$language_codes = array('client' => $language['value'], 'company' => $language['value']);
		
		// Set the client language
		if ($this->Session->read("blesta_client_id") && ($client = $this->Clients->get($this->Session->read("blesta_client_id")))) {
