...
class CurrencyFormat {
	 ...
	 * @param array $options An array of options to control the format (optional), if not set the options are pulled from the company settings. Options include:
	 ...
	 * 	- html_code Whether or not to wrap the currency code with HTML true/false (default false). If true, will produce currency code like <span class="currency_code">USD</span>
	 * 	- decimals The number of decimal places (precision), optional, default 2
	...
	public function format($value, $currency, array $options=null) {
		...
		$value = $this->Currencies->toCurrency($value, $currency, $this->company_id, $options['prefix'], $options['suffix'], $options['code'], $options['with_separator'], $options['decimals']);
	 ...
	 * @param int $decimals The number of decimals points (precision)
	 * @return string The value in decimal format based on ISO 31-0
	 * @see CurrencyFormat::format()
	 * @see CurrencyFormat::clear()
	 */
	public function cast($value, $currency, $decimals=2) {
